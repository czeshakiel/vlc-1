...
<div>
    ...
    <table border="1" width="100%" cellspacing="0" cellpadding="1" style="border-collapse: collapse; font-size:14px;">
        ...
        foreach($payroll_daily as $branch){
            ...
            $net=((420*$branch['no_of_days_work'])+$adjusttotal) - $fixdeduct - $deduct_total;
            $gross=(420*$branch['no_of_days_work'])+$adjusttotal;
            if($net < 0){
                $net=0;
            }
            if($branch['empid'] <> "06" && $branch['empid'] <> "07"){
            echo "<tr>";
                echo "<td>$x.</td>";
                echo "<td>$branch[lastname], $branch[firstname] $branch[middlename] $branch[suffix]</td>";                                
                echo "<td align='right'>".number_format(420,2)."</td>";
